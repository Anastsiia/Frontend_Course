...
function countryToLiveIn(language, isIsland, population, country) {
      if (typeof language !== 'string' || typeof isIsland !== 'boolean' ||
            typeof population !== 'number' || typeof country !== 'object') {
            return "Invalid arguments data type!";
      } else if (language != country.language ||
                  isIsland != country.isIsland || 
                  population < country.populationMln) {
            return `${country.name} does not meet your criteria`;
      } else {
            return `You should live in ${country.name}.`
      }
}
...
console.log(countryToLiveIn("English", false, 50, country));
